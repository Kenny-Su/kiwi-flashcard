...
export default function ConceptMapModal({ decks, initialDeckId, api, onClose, onEdit }: {
  ...
  const positions = useMemo(() => layoutCards(cards), [cards]);
  ...
  const overviewBox = (nextZoom = zoom): ViewBox => {
    const width = 900 / nextZoom; const height = 520 / nextZoom;
    return { x: (900 - width) / 2, y: (520 - height) / 2, width, height };
  };

  const resetFocus = () => { setSelectedLinkId(''); setEditing(false); animateCamera(overviewBox()); };

  const loadLinks = async (nextDeckId: string) => {
    if (!nextDeckId) { setLinks([]); return; }
    setLoading(true); setError(null);
    try { setLinks(await api.listCardLinks(nextDeckId)); }
  ...
  useEffect(() => { setSelectedCardId(cards[0]?.id || ''); setSuggestions([]); setConnectIds([]); setDraftPair([]); setSelectedLinkId(''); setZoom(1); setCamera(overview); }, [cards, deckId]);
  useEffect(() => {
  ...
  }, [connectMode, selectedLinkId, zoom]);
  ...
  const backgroundClick = (event: MouseEvent<SVGSVGElement>) => { if (event.target === event.currentTarget && selectedLinkId) resetFocus(); };
    ...
    <div className="concept-map">
      ...
      {!deck ? <div className="concept-map__empty">Create a deck before building a map.</div> : !cards.length ? <div className="concept-map__empty">This deck has no cards yet.</div> : <>
        <section className={`concept-map__canvas${selectedLink ? ' concept-map__canvas--focused' : ''}`}>
          <div className="concept-map__zoom"><button className="icon-button icon-button--small icon-button--surface" type="button" onClick={() => { const next = Math.min(1.6, zoom + .2); setZoom(next); animateCamera(overviewBox(next)); }} aria-label="Zoom in"><Icon name="add" size={15} /></button><button className="icon-button icon-button--small icon-button--surface" type="button" onClick={() => { const next = Math.max(.7, zoom - .2); setZoom(next); animateCamera(overviewBox(next)); }} aria-label="Zoom out">−</button></div>
          {loading ? <div className="concept-map__loading"><Spinner label="Loading map" /></div> : <svg viewBox={`${camera.x} ${camera.y} ${camera.width} ${camera.height}`} role="img" aria-label={`${cards.length} cards connected by ${links.length} relationships`} onClick={backgroundClick}>
            <g className="concept-map__edges">{links.map((link, index) => { const source = positions.get(link.sourceCardId); const target = positions.get(link.targetCardId); if (!source || !target) return null; const edge = edgePoints(source, target); const active = link.id === selectedLinkId; const muted = Boolean(selectedLink && !active); const sourceName = cardsById.get(link.sourceCardId)?.question || 'card'; const targetName = cardsById.get(link.targetCardId)?.question || 'card'; return <g className={`concept-map__edge${active ? ' concept-map__edge--selected' : ''}${muted ? ' concept-map__edge--muted' : ''}`} role="button" tabIndex={0} aria-label={`${sourceName} and ${targetName}: ${link.explanation}`} onClick={(event) => { event.stopPropagation(); focusLink(link); }} onKeyDown={(event) => edgeKey(event, link)} style={{ animationDelay: `${index * 45}ms` }} key={link.id}><line className="concept-map__edge-visible" {...edge} /><line className="concept-map__edge-hit" {...edge} /></g>; })}</g>
            <g className="concept-map__nodes">{cards.map((card, index) => { const position = positions.get(card.id)!; const lines = cardLabel(card.question); const picked = connectIds.includes(card.id); const muted = Boolean(focusedIds && !focusedIds.has(card.id)); return <g className={`concept-map__node${selectedCardId === card.id ? ' concept-map__node--selected' : ''}${picked ? ' concept-map__node--picked' : ''}${muted ? ' concept-map__node--muted' : ''}`} transform={`translate(${position.x - 78} ${position.y - 34})`} role="button" tabIndex={0} aria-label={card.question} onClick={(event) => { event.stopPropagation(); chooseNode(card.id); }} onKeyDown={(event) => nodeKey(event, card.id)} style={{ animationDelay: `${index * 35}ms` }} key={card.id}><rect width="156" height="68" rx="16" /><text x="78" y={lines.length === 1 ? 38 : 29} textAnchor="middle">{lines.map((line, i) => <tspan x="78" dy={i ? 17 : 0} key={line}>{line}</tspan>)}</text></g>; })}</g>
          </svg>}
